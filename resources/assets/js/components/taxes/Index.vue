            ...
            <div class="row">
              <table-byte :set-table="data">
                ...
                <table-row slot="table-row" slot-scope="{ item }">
                  ...
                  <table-cell>
                    <a
                      :href="`${url}admin/taxes/${item.id}/edit`"
                      class="btn-action"
                      title="Editar"
                    >
                    ...
                    <a
                      href="#!"
                      class="btn-action"
                      @click="_confirm(item, _deleted, 'eliminar')"
                      title="Eliminar"
                    >
...
export default {
  ...
  mounted() {
    this.url = urlBase;
    this.data = [...this.taxes];
    this.modal = M.Modal.init(document.querySelector(".modal"));
  },
  methods: {
    toCreate() {
      window.location = `${this.url}admin/taxes/create`;
    },
    ...
    _deleted() {
      ...
      axios
        .delete(`admin/taxes/${this.item.id}`)
        .then(res => {
    ...
    toggleStatus() {
      ...
      this.modal.close();
      axios
        .post(`admin/taxes/${this.item.id}/status`)
        .then(res => {
