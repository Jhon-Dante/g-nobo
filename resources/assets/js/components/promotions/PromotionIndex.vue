            ...
            <div class="row">
              <table-byte :set-table="data">
                <table-row slot="table-head" slot-scope="{ item }">
                  ...
                  <table-head>Usos por cliente</table-head>
                  <table-head>Estado de la promocion</table-head>
                  <table-head style="min-width: 110px;" class="center-align">Acciones</table-head>
                </table-row>
                <table-row slot="table-row" slot-scope="{ item }">
                  <table-cell>{{ item.title }}</table-cell>
                  <table-cell>{{ item.start_date | date }}</table-cell>
                  <table-cell>{{ item.end_date | date }}</table-cell>
                  <table-cell>{{ item.discount_percentage }}%</table-cell>
                  <table-cell>{{ item.limit }}</table-cell>
                  <table-cell>{{ item.status_name }}</table-cell>
                  <table-cell>
                    <a
                      :href="`${url}admin/promotions/${item.id}/edit`"
                      class="btn-action"
                      title="Editar"
                    >
                    ...
                    <div class="confirmation__text" v-if="item">
                      <h5 class="text-center">
                        ...
                        <b v-if="action == 'activar'">
                          {{
                          `${item.status == 2 ? "activar " : "desactivar"}`
                          }}
                        </b>
                        <b v-else>eliminar</b>
                        esta promocion?
...
export default {
  ...
  methods: {
    toCreate() {
      window.location = `${this.url}admin/promotions/create`;
    },
    ...
    _deleted() {
      ...
      axios
        .delete(`admin/promotions/${this.item.id}`)
        .then(res => {
    ...
    toggleStatus() {
      ...
      this.modal.close();
      axios
        .post(`admin/promotions/${this.item.id}/status`)
        .then(res => {
