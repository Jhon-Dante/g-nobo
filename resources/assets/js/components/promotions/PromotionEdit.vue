      ...
      <div class="col s12">
        ...
        <card-main>
          <card-content>
            <div class="row">
              <div class="col s12">
                <h5 class="center-align">Editar Promocion</h5>
            ...
            <div class="row">
              ...
              <div class="col s4 center-align div-image-margin-top">
                ...
                <input-file v-if="form.image" :file="`${form.image}` | img" :btn="false" :image="true" @file="_setFile($event)"></input-file>
                <input-file v-else :btn="false" :image="true" @file="_setFile($event)"></input-file>
              </div>
            </div>
            <div class="row">
              <div class=" col s4">
...
export default {
  ...
  props: ["categories", "promotion", "productAmounts"],
  data() {
    return {
        sending: false,
        category_id: "",
    ...
    productsAdded() {
        ...
        if(this.selectedProduct) {
            if(this.selectedProduct.colors.length > 0) {
                ...
                }).filter((amount) => !this.products_ids.includes(amount.product_id))
  ...
  methods: {
    ...
    toggle(product_amount) {
        ...
        if(index >= 0){
            const indexId = this.products_ids.findIndex((product_id) => product_amount.product_id == product_id);
            const currentProduct = this.products[index];
            currentProduct.total = 0;
            this.products.splice(index, 1);
            this.products_ids.splice(indexId, 1);
            if(currentProduct.master_id == this.selectedProduct.id) {
              const index = this.productsAdded.findIndex((product) => currentProduct.master_id == this.selectedProduct.id)
              if(index < 0) {
    ...
    submit() {
      ...
      this.sending = true;
      axios
        .post(`admin/promotions/${this.promotion.id}`, this._convertToFormData())
        .then((res) => {
  ...
  mounted() {
    ...
    let productAmountsFormatted = [];
    for(let i=0; i < this.promotion.products.length; i++) {
      const foundProduct = this.productAmounts.find((itmInner) => itmInner.id === this.promotion.products[i].product_id);
      if(foundProduct) {
        productAmountsFormatted.push({
        ...foundProduct,
        name: foundProduct.product.name,
        product_id: this.promotion.products[i].product_id,
        total: this.promotion.products[i].amount,
        existence: foundProduct.amount
        });
      }
    }
    this.products_ids = productAmountsFormatted.map((product) => product.id);
    this.products = productAmountsFormatted;
    ...
    var instancesStart = M.Datepicker.init(startDate, {
      ...
      onClose: this.verify,
      minYear: minYear,
      defaultDate: moment(this.form.start_date).toDate(),
      setDefaultDate: true,
      i18n: pickDateI18n,
      minDate: new Date(),
    });
    var instanceEnd = M.Datepicker.init(endDate, {
      ...
      minYear: minYear,
      i18n: pickDateI18n,
      defaultDate: moment(this.form.end_date).toDate(),
      setDefaultDate: true,
      minDate: new Date(),
    });
  },
