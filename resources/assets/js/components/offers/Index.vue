            ...
            <div class="row">
              <div class="col s12 container-btn-add">
                <button class="btn-add" @click="toCreate">
                  <img
                    :src="'img/icons/new-msg.png' | asset"
                    alt
            ...
            <div class="row">
              <table-byte :set-table="data">
                <table-row slot="table-head" slot-scope="{ item }">
                  ...
                  <table-head>Fecha de finalización</table-head>
                ...
                <table-row slot="table-row" slot-scope="{ item }">
                  ...
                  <table-cell>
                    <a
                      :href="`${url}admin/offers/${item.id}/edit`"
                      class="btn-action"
                      title="Editar"
                    >
                      <img
                        :src="'img/icons/ico-editar.png' | asset"
                        alt
                        class="img-responsive"
                      />
                    </a>
                    ...
                    <!-- <a
                      href="#!"
                      class="btn-action"
                      @click="_confirm(item, _deleted)"
                      title="Eliminar"
                    >
                      <img
                        :src="'img/icons/ico-eliminar.png' | asset"
                        alt
                        class="img-responsive"
                      />
                    </a> -->
...
export default {
  name: "offer-index",
  props: ["offers", "categories", "products"],
  components: {
    Create,
    Edit,
  },
  ...
  mounted() {
    this.url = urlBase;
    this.data = [...this.offers];
    this.modal = M.Modal.init(document.querySelector(".modal"));
  },
  methods: {
    toCreate() {
      window.location = `${this.url}admin/offers/create`;
    },
    _confirm(item, method) {
      this.item = item;
      this.option = method;
      this.modal.open();
    },
    ...
    _deleted() {
      ...
      axios
        .delete(`admin/offers/${this.item.id}`)
        .then((res) => {
    ...
    toggleStatus() {
      ...
      this.modal.close();
      axios
        .post(`admin/offers/${this.item.id}/status`)
        .then((res) => {
